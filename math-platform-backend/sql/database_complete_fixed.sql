...
(
    ...
    followCount    int          default 0                 not null comment '关注数',
    fanCount       int          default 0                 not null comment '粉丝数',
...
-- 修复 user 表字段顺序
ALTER TABLE user 
MODIFY COLUMN userAccount varchar(50) NOT NULL COMMENT '用户账号' AFTER id,
MODIFY COLUMN userPassword varchar(100) NOT NULL COMMENT '用户密码' AFTER userAccount,
MODIFY COLUMN userName varchar(100) NOT NULL COMMENT '用户昵称' AFTER userPassword,
MODIFY COLUMN userAvatar varchar(500) NULL COMMENT '用户头像' AFTER userName,
MODIFY COLUMN userProfile varchar(1000) NULL COMMENT '用户简介' AFTER userAvatar,
MODIFY COLUMN userRole varchar(50) NOT NULL DEFAULT 'user' COMMENT '用户角色：user/admin/ban' AFTER userProfile,
MODIFY COLUMN gender tinyint NOT NULL DEFAULT 0 COMMENT '性别：0-未知 1-男 2-女' AFTER userRole,
MODIFY COLUMN email varchar(100) NULL COMMENT '邮箱' AFTER gender,
MODIFY COLUMN phone varchar(20) NULL COMMENT '手机号' AFTER email,
MODIFY COLUMN birthday datetime NULL COMMENT '生日' AFTER phone,
MODIFY COLUMN location varchar(100) NULL COMMENT '所在地' AFTER birthday,
MODIFY COLUMN website varchar(200) NULL COMMENT '个人网站' AFTER location,
MODIFY COLUMN followingCount int NOT NULL DEFAULT 0 COMMENT '关注数' AFTER website,
MODIFY COLUMN followerCount int NOT NULL DEFAULT 0 COMMENT '粉丝数' AFTER followingCount,
MODIFY COLUMN postCount int NOT NULL DEFAULT 0 COMMENT '发帖数' AFTER followerCount,
MODIFY COLUMN likeCount int NOT NULL DEFAULT 0 COMMENT '获赞数' AFTER postCount,
MODIFY COLUMN status tinyint NOT NULL DEFAULT 1 COMMENT '状态：0-封禁 1-正常' AFTER likeCount,
MODIFY COLUMN lastLoginTime datetime NULL COMMENT '最后登录时间' AFTER status,
MODIFY COLUMN lastLoginIp varchar(50) NULL COMMENT '最后登录IP' AFTER lastLoginTime,
MODIFY COLUMN editTime datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '编辑时间' AFTER lastLoginIp,
MODIFY COLUMN createTime datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间' AFTER editTime,
MODIFY COLUMN updateTime datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间' AFTER createTime,
MODIFY COLUMN isDelete tinyint NOT NULL DEFAULT 0 COMMENT '是否删除：0-未删除 1-已删除' AFTER updateTime;

-- 修复 post 表字段顺序
ALTER TABLE post
MODIFY COLUMN title varchar(200) NOT NULL COMMENT '帖子标题' AFTER id,
MODIFY COLUMN content text NOT NULL COMMENT '帖子内容' AFTER title,
MODIFY COLUMN contentSummary varchar(500) NULL COMMENT '内容摘要' AFTER content,
MODIFY COLUMN userId bigint NOT NULL COMMENT '发布者ID' AFTER contentSummary,
MODIFY COLUMN category varchar(50) NOT NULL DEFAULT 'tech' COMMENT '分类：tech/question/project/share' AFTER userId,
MODIFY COLUMN tags varchar(500) NULL COMMENT '标签，JSON格式' AFTER category,
MODIFY COLUMN images varchar(2000) NULL COMMENT '图片URL，JSON格式' AFTER tags,
MODIFY COLUMN viewCount int NOT NULL DEFAULT 0 COMMENT '浏览量' AFTER images,
MODIFY COLUMN likeCount int NOT NULL DEFAULT 0 COMMENT '点赞数' AFTER viewCount,
MODIFY COLUMN commentCount int NOT NULL DEFAULT 0 COMMENT '评论数' AFTER likeCount,
MODIFY COLUMN favouriteCount int NOT NULL DEFAULT 0 COMMENT '收藏数' AFTER commentCount,
MODIFY COLUMN shareCount int NOT NULL DEFAULT 0 COMMENT '分享数' AFTER favouriteCount,
MODIFY COLUMN qualityScore decimal(3,2) NOT NULL DEFAULT 0.00 COMMENT '质量评分' AFTER shareCount,
MODIFY COLUMN hotScore decimal(10,2) NOT NULL DEFAULT 0.00 COMMENT '热度评分' AFTER qualityScore,
MODIFY COLUMN status tinyint NOT NULL DEFAULT 1 COMMENT '状态：0-删除 1-正常 2-审核中 3-精华' AFTER hotScore,
MODIFY COLUMN isTop tinyint NOT NULL DEFAULT 0 COMMENT '是否置顶' AFTER status,
MODIFY COLUMN isHot tinyint NOT NULL DEFAULT 0 COMMENT '是否热门' AFTER isTop,
MODIFY COLUMN auditStatus tinyint NOT NULL DEFAULT 1 COMMENT '审核状态：0-待审核 1-通过 2-拒绝' AFTER isHot,
MODIFY COLUMN auditTime datetime NULL COMMENT '审核时间' AFTER auditStatus,
MODIFY COLUMN auditUserId bigint NULL COMMENT '审核人ID' AFTER auditTime,
MODIFY COLUMN auditRemark varchar(500) NULL COMMENT '审核备注' AFTER auditUserId,
MODIFY COLUMN publishTime datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '发布时间' AFTER auditRemark,
MODIFY COLUMN createTime datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间' AFTER publishTime,
MODIFY COLUMN updateTime datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间' AFTER createTime,
MODIFY COLUMN isDelete tinyint NOT NULL DEFAULT 0 COMMENT '是否删除：0-未删除 1-已删除' AFTER updateTime;
...
-- ========================================
-- 第四部分: 测试数据插入（仅在数据为空时插入）
-- ========================================

-- 检查是否需要插入测试数据
SET @user_count = (SELECT COUNT(*) FROM user WHERE userAccount != 'admin');
...
SELECT * FROM (
    SELECT '深入理解微积分的本质：从极限到导数的数学之美' as title,
    ...
    SELECT '线性代数在机器学习中的应用实例',
           ...
           980, 38, 15, 22, 1
    UNION ALL
    SELECT 'React 18新特性深度解析：并发渲染的实现原理',
           ...
           'tech',
           '["React","前端","并发渲染","JavaScript"]',
           756, 29, 8, 15, 1
    UNION ALL
    SELECT '求助：如何理解拉格朗日乘数法的几何意义？',
